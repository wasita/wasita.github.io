<script lang="ts">
  type Post = {
    meta: { title: string; date: string; excerpt?: string; visible?: boolean };
    path: string;
  };
...
<div class="px-4 lg:px-6 pt-12 pb-16 max-w-6xl mx-auto">
  <div class="grid grid-cols-12 gap-y-8 lg:gap-10 items-center">
    ...
    <div class="col-span-12 lg:col-span-8 order-2 lg:order-1">
      <h1
        class="font-display mb-0"
        style="font-size: clamp(3rem, 9vw, 6.5rem); line-height: 0.95;"
      >
        hi, i'm<br />
        <span
          style="color: var(--accent); position: relative; display: inline-block;"
        >
          wasita<span
            class="sparkle"
            aria-hidden="true"
            style="position: absolute; top: -0.3em; right: -0.25em; width: 0.3em; height: 0.3em;"
          ></span><span
            class="sparkle"
            aria-hidden="true"
            style="position: absolute; bottom: 0.15em; left: -0.25em; width: 0.22em; height: 0.22em;"
          ></span></span
        ><span
          class="saturn-orb"
          aria-hidden="true"
          style="width: 0.55em; height: 0.55em; margin-left: 0.05em; vertical-align: 0.15em;"
        ></span><span
          class="sparkle"
          aria-hidden="true"
          style="width: 0.18em; height: 0.18em; margin-left: 0.05em; vertical-align: 0.55em;"
        ></span>.
      </h1>
      <p
        class="font-body text-xl lg:text-2xl leading-tight mt-5 lg:mt-7"
    ...
    <div
      class="col-span-12 lg:col-span-4 order-1 lg:order-2 flex justify-center lg:justify-end mb-8 lg:mb-0"
    >
...
{#if latestPost || drafts.length > 0}
  <section class="px-4 lg:px-6 pb-20 max-w-6xl mx-auto">
    <h2
      class="font-display mb-6 lg:mb-8"
      style="font-size: clamp(2.25rem, 5vw, 3.5rem); line-height: 1; color: var(--ink);"
    >
      latest <span style="color: var(--accent);">musings</span><span
        class="star"
        aria-hidden="true"
        style="width: 0.5em; height: 0.5em; margin-left: 0.2em; vertical-align: 0.5em;"
      ></span>
    </h2>
    ...
    <div class="max-w-3xl">
      {#if latestPost}
        <a href="/{latestPost.path}" class="block mb-6 no-underline group">
          ...
          <h3
            class="font-display mb-3"
            style="font-size: clamp(1.75rem, 3.5vw, 2.5rem); line-height: 1; color: var(--ink);"
          >
            <span
              class="starry-link group-hover:[bg-size:100%_3px]"
              style="background-image: linear-gradient(var(--accent), var(--accent)); background-repeat: no-repeat; background-position: 0 100%; background-size: 100% 2px; transition: background-size 0.3s ease; padding-bottom: 4px;"
            >
          ...
          {#if latestPost.meta.excerpt}
            <p
              class="font-body text-lg leading-relaxed"
      ...
      {#if drafts.length > 0}
        <p
          class="font-body text-sm leading-relaxed mt-4"
          style="color: var(--ink-soft);"
        >
          <span
            class="font-marker text-lg"
            style="color: var(--accent); margin-right: 0.5rem;"
            >↳ in the works:</span
          >
          {#each drafts as d, i}<em style="color: var(--ink);"
              >{d.meta.title}</em
            ><span
              class="font-marker"
              style="color: var(--caption-ink); margin-left: 0.3rem; font-size: 0.95rem;"
              >(draft)</span
            >{#if i < drafts.length - 1}{", "}{/if}{/each}
        </p>
      {/if}
    </div>
  </section>
