...
{#if latestPost || drafts.length > 0}
 ...
 {#each drafts as d, i}<em style="color: var(--ink);">{d.meta.title}</em><span class="font-marker" style="color: var(--caption-ink); margin-left: 0.3rem; font-size: 0.95rem;">(draft)</span>{#if i < drafts.length - 1}{', '}{/if}{/each}
 </p>
 {/if}
 </div>
 </section>
